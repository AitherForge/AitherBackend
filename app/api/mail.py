...
async def list_messages(start: int = 0, limit: int = 100, aither_session: str | None = Cookie(default=None, alias=SESSION_COOKIE), authorization: str | None = Header(default=None)):
    user = _user(aither_session, authorization); start, limit = max(0,start), min(100,max(1,limit))
...
async def delete_message(message_id: str, aither_session: str | None = Cookie(default=None, alias=SESSION_COOKIE), authorization: str | None = Header(default=None)):
    user = _user(aither_session, authorization)
    with connection() as conn: changed = conn.execute("UPDATE mail_messages SET deleted = 1 WHERE id = ? AND owner_user_id = ?",(message_id,str(user["id"]))).rowcount
...
async def send_message(payload: SendMail, aither_session: str | None = Cookie(default=None, alias=SESSION_COOKIE), authorization: str | None = Header(default=None)):
    user = _user(aither_session, authorization); sender = user["email"]
    ...
    now=datetime.now(timezone.utc).isoformat(); recipient_text=", ".join(recipients)
    with connection() as conn:
        cur=conn.execute("INSERT INTO mail_messages(owner_user_id,sender,recipients,cc,subject,body,created_at,is_read,deleted,folder) VALUES(?,?,?,?,?,?,?,?,?,?)",(str(user["id"]),sender,recipient_text,", ".join(cc),payload.subject,payload.body,now,1,0,"sent")); sent_id=str(cur.lastrowid)
        local_emails={x.lower() for x in recipients+cc+bcc}
        if local_emails:
            placeholders=",".join("?" for _ in local_emails); rows=conn.execute(f"SELECT id,email FROM users WHERE lower(email) IN ({placeholders})",tuple(local_emails)).fetchall()
            for target in rows: conn.execute("INSERT INTO mail_messages(owner_user_id,sender,recipients,cc,subject,body,created_at,is_read,deleted,folder) VALUES(?,?,?,?,?,?,?,?,?,?)",(str(target["id"]),sender,recipient_text,", ".join(cc),payload.subject,payload.body,now,0,0,"inbox"))
    smtp_host=os.getenv("SMTP_HOST","").strip(); external=[x for x in recipients+cc+bcc if x.lower()!=sender.lower()]
