...
async def send_message(payload: SendMail, aither_session: str | None = Cookie(default=None, alias=SESSION_COOKIE), authorization: str | None = Header(default=None)):
    ...
    from_email = os.getenv("RESEND_FROM_EMAIL", settings.smtp_from_email).strip()
    if not from_email:
        raise HTTPException(status_code=503, detail="Email delivery is not configured: RESEND_FROM_EMAIL is missing.")
    from_address = f"{from_name} <{from_email}>"
    params: resend.Emails.SendParams = {"from": from_address, "to": recipients, "subject": payload.subject, "text": payload.body, "reply_to": sender}
    if cc: params["cc"] = cc
