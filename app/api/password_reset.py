...
async def send_reset_email(name: str, email: str, token: str) -> None:
    ...
    html_body = (
        '<!doctype html><html><body style="margin:0;background:#080d17;color:#f7f9ff;font-family:Arial,Helvetica,sans-serif">'
        '<table width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center" style="padding:40px 16px">'
        '<table width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;background:#101b2d;border:1px solid #294568;border-radius:20px">'
        '<tr><td style="padding:36px"><h1 style="font-size:28px;line-height:1.2;color:#28a9ff">Reset your Aither Account password</h1>'
        '<p style="font-size:16px;line-height:1.6;color:#d8e4f7">Hi ' + safe_name + ',</p>'
        '<p style="font-size:15px;line-height:1.7;color:#aebed5">We received a request to reset your Aither Account password. Use the button below to choose a new password.</p>'
        '<p style="text-align:center;padding:18px 0"><a href="' + safe_link + '" style="display:inline-block;padding:15px 28px;background:#159ff4;color:#fff;text-decoration:none;border-radius:12px;font-weight:700">Reset Password</a></p>'
        '<p style="font-size:12px;line-height:1.6;color:#879bb5">This link expires in 1 hour. If you did not request a password reset, you can safely ignore this email.</p>'
        '</td></tr></table></td></tr></table></body></html>'
    )
    text = f"Hi {name},\n\nReset your Aither Account password here:\n{link}\n\nThis link expires in 1 hour. If you did not request this, ignore this email."
    resend.api_key = key
    await resend.Emails.send_async(
        {"from": sender, "to": [email], "subject": "Reset your Aither Account password", "html": html_body, "text": text}
    )
...
async def reset_password(payload: ResetPasswordRequest) -> dict[str, bool]:
    with connection() as conn:
        ...
        conn.execute(
            ...
            (row["user_id"], "password_reset", now().isoformat()),
        )
    return {"ok": True}
